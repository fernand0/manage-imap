...
class EmailManager:
    ...
    def _display_menu_and_get_choice(self, menu_title: str, menu_options: List[str]) -> int:
    ...
    def select_message(self) -> Optional[Any]:
        ...
        try:
            ...
            if not posts:
                print(f"No messages found in current folder: {self.api_src.getChannel()}")
                return None
            ...
            while True:
                try:
                    choice = input("Select message number (or 'q' to quit): ").strip()
                    if choice.lower() == 'q':
                        return None
                    
                    msg_num = int(choice)
                    if 0 <= msg_num < len(posts[-15:]):
                        return posts[-(15-msg_num)]
                    else:
    ...
    def move_message(self, create_rule: bool = False) -> None:
        ...
        try:
            ...
            (keyword, textt, textHeader) = self.api_src.selectHeaderAuto(self.api_src, msg)
            ...
            if 'Subject' not in keyword and '@' in textHeader:
                ...
                textHeaderS = next((part for part in domain.split('.') if part != 'www'), textHeaderS)

            folder = self.api_src.selectFolderN(self.api_src.getClient(), folderM=textHeaderS)
            if not folder:
    ...
    def _apply_rule_logic(self, rule: Tuple, interactive: bool) -> None:
        ...
        logging.info(f"Applying rule: moving messages matching '{search_criteria}' to '{folder}'")
        ...
        try:
            ...
            _, msg_ids = self.api_src.getClient().search('utf-8', search_criteria.encode('utf-8'))
    ...
    def _select_rule(self) -> Optional[Tuple]:
        ...
        while True:
            cat_choice = input(f"Select category ({'/'.join(categories)}) or 'q' to quit: ").lower()
            if cat_choice == 'q': return None
        ...
        while True:
            try:
                rule_num_str = input(f"Select rule number (0-{len(rules_in_cat)-1}) or 'q' to quit: ").strip()
                if rule_num_str.lower() == 'q': return None
                rule_num = int(rule_num_str)
                if 0 <= rule_num < len(rules_in_cat):
                    return rules_in_cat[rule_num]
                else:
    ...
    def organize_rules(self) -> None:
        ...
        if not original_category:
            logging.error("Could not find the selected rule in any category. This should not happen.")
            return

        print(f"\nSelected rule: {rule_to_move} (from '{original_category}')")

        dest_categories = [cat for cat in self.rule_manager.rules.keys() if cat != original_category] + ['delete']
        while True:
            dest_choice = input(f"Move to category ({'/'.join(dest_categories)})? ").lower()
    ...
    def purge_deleted_mails(self) -> None:
        ...
        print(f"This will permanently delete all emails marked for deletion in the folder '{current_folder}'.")
        ...
        try:
            ...
            if typ == 'OK':
                ...
                print(f"Successfully purged {purged_count} message(s) from '{current_folder}'.")
            else:
                logging.error(f"Failed to expunge folder. Server response: {typ} {data}")
        except Exception as e:
